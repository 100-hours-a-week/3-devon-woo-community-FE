...
export const generateMockPosts = (count: number = 20): PostSummaryResponse[] => {
  return Array.from({ length: count }, (_, i) => ({
    postId: i + 1,
    title: mockTitles[i % mockTitles.length],
    summary: mockSummaries[i % mockSummaries.length],
    thumbnail: `https://picsum.photos/seed/post${i + 1}/800/400`,
    member: {
      ...baseMember,
      memberId: (i % 3) + 1,
...
export const generateMockPost = (id: number): PostResponse => {
  const index = (id - 1) % mockTitles.length
  return {
    postId: id,
    ...
    tags: ['JavaScript', 'React', 'TypeScript', 'Node.js'].slice(0, (id % 4) + 1),
    member: {
      ...baseMember,
      memberId: (id % 3) + 1,
